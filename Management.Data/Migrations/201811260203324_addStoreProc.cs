namespace Management.Data.Migrations
{
    ...
    public partial class addStoreProc : DbMigration
    {
        public override void Up()
        {
            DropStoredProcedure("Proc_Status_Seat");
            CreateStoredProcedure("Proc_Status_Seat",
               p => new
               {
                   IDCar = p.Int(),
                   DateBook = p.String()
    ...
    with List as (
    	                        Select a.Row, Row_number() over(partition By a.ID order by a.ID) as [Seats], a.IDCar, a.ID,b.Status
    	                        from Seats a join SeatNos b on a.ID = b.IDSeat
    	                        where a.IDCar = @IDCar and b.DateBook >= cast(@DateBook as date) and b.DateBook <= cast(DATEADD(day, 1, @DateBook) as date)
    )
	...
	where c.DateBook >= cast(@DateBook as date) and c.DateBook<=cast(DATEADD(day, 1, @DateBook) as date)
